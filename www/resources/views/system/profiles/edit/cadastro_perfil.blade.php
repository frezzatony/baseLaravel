<div class="row p-0 m-0 mb-1">
    ...
    <div class="col-md-5 py-0 px-1">
        ...
        <select id="is_active" name="is_active" class="form-select px-1 pt1 pb1 fs-sm">
            <option value="t" {{ $profile->is_active ? 'selected' : '' }}>SIM</option>
            <option value="f" {{ !$profile->is_active ? 'selected' : '' }}>NÃO</option>
        </select>
    </div>
</div>
<div class="row p-0 m-0 mb-1">
    <div class="col-md-12 py-0 px-1">
        ...
        <input type="text" id="name" name="name" class="form-control px-1 pt1 pb1 fs-sm uppercase" value="{{ $profile->name }}" {!! $profile->can_edit ? '' : 'readonly' !!}>
    </div>
</div>
