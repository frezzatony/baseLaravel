...
(function (factory) {
    ...
    init: function (options) {
      return this.each(function () {
        ...
        }
        {
          ...
          // Initiate input
          input.data('inputpicker-uuid', uuid).addClass('inputpicker-input').prop('id', 'inputpicker-' + uuid).prop('name', 'inputpicker-' + uuid);

          // Inputpicker div ( wrap fake input and arrow )
          var inputpicker_div = $("<div id=\"inputpicker-div-" + uuid + "\" class=\"inputpicker-div\" data-uuid=\"" + uuid + "\" style=\"position:relative;overflow:auto;height:100%;\"></div>")
            .append(input)
            .on('click', function (e) {
        ...
        if (settings.url != undefined && settings.autoload != false) {
  ...
  function _execJSON(input, param, func) {
    ...
    var paramJson = JSON.stringify(param).replace("{q}", input.val());
  ...
  function _getWrappedList(input) {
    ...
    if (!wrapped_list.length) {
      wrapped_list = $('<div />', {
        ...
        'position': 'absolute',
        'overflow': 'auto'
      }).addClass('inputpicker-wrapped-list shadow')
  ...
  function _renderTableHeader(input) {
    ...
    if (_set(input, 'headShow')) {
      ...
      for (var i = 0; i < fields.length; i++) {
        ...
        output += '<th class="bg-light f8 border">' + text + '</th>';
      }
      output += '</thead>';
    }
    return output;
    ...
    if (data.length) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        for (var j = 0; j < fields.length; j++) {
          ...
          text = (typeof fields[j] == 'object' && fields[j]['format'] != undefined) ? fields[j]['format'](text) : text;
          ...
          output += ' class="pa1 f8 border">' + text + '</td>';
        }
        output += '</tr>';
      }
      ...
      output += "<thead><tr><td colspan='" + fields.length + "' align='center' class=\"inputpicker-no-result\">Sem resultados.</td></thead></tr>";
    }
  ...
  function _dataRenderDefault(input) {
    ...
    // Draw table
    output += "<table class=\"table small table-sm table-striped bg-white\">";
    ...
    if (data.length) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        for (var j = 0; j < fields.length; j++) {
          ...
          output_tds += ' class="pa1 f8 border">' + text + '</td>';
        }
      ...
      output += "<thead><tr><td colspan='" + fields.length + "' align='center'>Sem resultados.</td></thead></tr>";
    }
  ...
  function _setValue(input, value) {
    ...
    if (_typeIsMultiple(input)) {
      ...
      if (index_i > -1) {
        ...
        original.data('response', data[index_i]);
